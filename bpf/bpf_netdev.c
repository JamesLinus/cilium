...
static inline int rewrite_dmac_if_needed(struct __sk_buff *skb)
{
#ifdef FROM_HOST
...
static inline int handle_ipv6(struct __sk_buff *skb)
{
	...
	/* If we are attached to cilium_host at egress, this will rewrite the
	 * destination mac address to the MAC of cilium_net */
	ret = rewrite_dmac_if_needed(skb);
	if (IS_ERR(ret))
		return ret;
	...
	return rewrite_dmac_if_needed(skb);
}
